...
public class GUIDRefReplace : EditorWindow
{
	...
	[MenuItem("gametools/RefFind/GUIDRefReplaceWin")]   // 菜单开启并点击的   处理
	...
	void OnGUI()
	{
		...
		if (GUILayout.Button ("自动同名匹配")) {
			if (_sourceOld == null) {
				EditorUtility.DisplayDialog ("error","_sourceOld is null","OK");
				return;
			}
			string filename = new FileInfo (AssetDatabase.GetAssetPath (_sourceOld)).Name;
			 
			int findCount = 0;

			Debug.LogError ("find:"+filename);

			foreach (var item in 			Directory.GetFiles(Application.dataPath, filename, SearchOption.AllDirectories)) {
				Object findItem =	AssetDatabase.LoadAssetAtPath (GetRelativeAssetsPath(item), _sourceOld.GetType ());
				Debug.LogError ("find:"+item);
				Debug.LogError (findItem);
				if (findItem == null||findItem==_sourceOld)
					continue;
					...
					if (findCount > 1) {
						Debug.LogError (findItem);
						_sourceNew = null;
						EditorUtility.DisplayDialog ("error", "同名资源超过1个 需要手动设置", "OK");
						return;
					}
					_sourceNew = findItem;
					Debug.LogError ("find:"+item);
